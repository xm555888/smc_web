...
async function createRedirectRenderResult(req, res, originalHost, redirectUrl, redirectType, basePath, workStore) {
    ...
    if (appRelativeRedirectUrl) {
        ...
        if (workStore.revalidatedTags) {
            ...
            forwardedHeaders.set(_constants.NEXT_CACHE_REVALIDATED_TAGS_HEADER, workStore.revalidatedTags.join(','));
...
async function handleAction({ req, res, ComponentMod, serverModuleMap, generateFlight, workStore, requestStore, serverActions, ctx }) {
    ...
    try {
        await actionAsyncStorage.run({
                ...
                if (isMultipartAction) {
                    ...
                    if (isFetchAction) {
                        ...
                        if (typeof action === 'function') {
                            ...
                            const actionReturnedState = await _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, action);
            ...
            actionId];
            const returnVal = await _workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, ()=>actionHandler.apply(null, boundActionArguments));
 ...
 */ function getActionModIdOrError(actionId, serverModuleMap) {
    try {
        ...
        if (!actionId) {
            throw Object.defineProperty(new Error("Invariant: Missing 'next-action' header."), "__NEXT_ERROR_CODE", {
                value: "E416",
                enumerable: false,
                configurable: true
            });
        ...
        if (!actionModId) {
            throw Object.defineProperty(new Error("Invariant: Couldn't find action module ID from module map."), "__NEXT_ERROR_CODE", {
                value: "E32",
                enumerable: false,
                configurable: true
            });
        ...
        throw Object.defineProperty(new Error(`Failed to find Server Action "${actionId}". This request might be from an older or newer deployment. ${err instanceof Error ? `Original error: ${err.message}` : ''}\nRead more: https://nextjs.org/docs/messages/failed-to-find-server-action`), "__NEXT_ERROR_CODE", {
            value: "E264",
            enumerable: false,
            configurable: true
        });
